...
public class TransferNode extends Node {
	private static Logger logger = LoggerFactory.getLogger(TransferNode.class);
	...
	// private static final String CFG_FROM_OBJECT_TYPE_KEY = "fromObjectType";
	// //源对象的类型存放的Key
	// private static final String CFG_FROM_OBJECT_CLASS_KEY = "fromObjectClass";
	// //源对象的类存放的Key
	...
	// private static final String CFG_TO_OBJECT_TYPE_KEY = "toObjectType";
	// //目标对象的类型存放的Key
	// private static final String CFG_TO_OBJECT_CLASS_KEY = "toObjectClass";
	// //目标对象的类存放的Key
	...
	public void process(BusinessFlowNode businessFlowNode, BusinessFlowContext context) throws EngineException {
		...
		// String toObjectType = nodeCfg, CFG_TO_OBJECT_TYPE_KEY);
		// String toObjectClass = nodeCfg, CFG_TO_OBJECT_CLASS_KEY);
		String dataMappingId = JsonUtils.getString(nodeCfg, CFG_DATA_MAPPING_ID_KEY);
		
		if (StringUtils.isBlank(fromObjectInstanceSource) || StringUtils.isBlank(fromObjectInstanceKey)) {
			logger.error("fT[{}] romObjectInstanceSource and fromObjectInstanceKey can't be null. businessflowNodeId: {}", businessFlowNode.getTenantId(), businessFlowNode.getId());
		...
		if (StringUtils.isBlank(dataMappingId)) {
			logger.error("T[{}] dataMappingId can't be null. businessflowId: {}", businessFlowNode.getTenantId(), businessFlowNode.getId());
			throw new EngineException(ResultStatus.INTERNAL_ERROR, "dataMappingId can't be null. ");
		}
		...
		// 获取源对象实例
		Object fromObject = getContextObject(fromObjectInstanceSource, fromObjectInstanceKey, isListType,
				fromObjectInstanceAttr, context, businessFlowNode);
		...
		if (StringUtils.isBlank(toObjectInstanceSource) || StringUtils.isBlank(toObjectInstanceKey)) {
			...
			if (isListType) {
				logger.error("T[{}] when isListType is true, toObjectInstanceSource and toObjectInstanceKey must be null. businessflowNodeId: {}", businessFlowNode.getTenantId(), businessFlowNode.getId());
		...
		if (isListType) {// 列表类型
			...
			DataTransferAbility.getInstance().transfer(context.getSystemId(), dataMappingId, ((JsonArray) fromObject),
					((JsonArray) toObject));
			...
			DataTransferAbility.getInstance().transfer(context.getSystemId(), dataMappingId, ((JsonObject) fromObject), ((JsonObject) toObject));
		}
		// 将目标对象保存到节点处理结果中
