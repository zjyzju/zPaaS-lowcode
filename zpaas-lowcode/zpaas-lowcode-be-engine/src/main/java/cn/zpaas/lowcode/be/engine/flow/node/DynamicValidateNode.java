	...
	/*
	 {
		...
		targetObjectInstanceSource：目标数据对象实例的来源，包括：I（输入参数）；P（过程数据）；D（领域对象）；N（预处理产生的nodeParams）
		targetObjectInstanceKey：目标数据对象实例的Key值，源对象类型为I（输入参数）时，表示输入参数中的Key；为D（领域对象）时，表示领域对象在context.attributedObjectMap或context.attributedObjectsMap（根据isListType的值进行区分）中的key值；为P（过程数据）时，该值表示context.values中的key值，当为N（预处理产生的nodeParams）时，Key值无效
		...
		dynamicMappingId：动态映射标识，选择映射类型为V（校验规则）的动态映射数据。
