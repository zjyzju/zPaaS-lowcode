...
/**
 * @author zjy 
 * ExcelImportNode是Excel文件解析导入的实现类，主要用于实现Excel文件的解析和处理能力
 */
public class ExcelImportNode extends Node {
	private static Logger logger = LoggerFactory.getLogger(ExcelImportNode.class);
	
	private static final String CFG_EXCEL_FILE_SOURCE_KEY = "excelFileSource"; //excel文件的来源存放的Key
	private static final String CFG_EXCEL_FILE_NAME_SOURCE_KEY = "excelFileNameSource"; //excel文件名的来源存放的Key
	private static final String CFG_EXCEL_FILE_NAME_KEY_KEY = "excelFileNameKey"; //excel文件名的Key值存放的Key
	private static final String CFG_EXCEL_FILE_NAME_ATTR_KEY = "excelFileNameAttr"; //当excel文件名是指定对象的某个属性时有效，通过该字段指定对应属性的code存放的Key
	private static final String CFG_SHEET_INDEX_KEY = "sheetIndex"; //待处理的sheet页号存放的Key
	...
	public void process(BusinessFlowNode businessFlowNode, BusinessFlowContext context) throws EngineException {
		...
		String excelFileSource = JsonUtils.getString(nodeCfg, CFG_EXCEL_FILE_SOURCE_KEY);
		String excelFileNameSource = JsonUtils.getString(nodeCfg, CFG_EXCEL_FILE_NAME_SOURCE_KEY);
		String excelFileNameKey = JsonUtils.getString(nodeCfg, CFG_EXCEL_FILE_NAME_KEY_KEY);
		String excelFileNameAttr = JsonUtils.getString(nodeCfg, CFG_EXCEL_FILE_NAME_ATTR_KEY);
		...
		//配置参数合法性校验
		if(StringUtils.isBlank(excelFileSource) || StringUtils.isBlank(excelFileNameSource) || StringUtils.isBlank(excelFileNameKey) ) {
			logger.error("T[{}] excelFileSource excelFileNameSource and excelFileNameKey can't be null. businessflowNodeId: {}", businessFlowNode.getTenantId(), businessFlowNode.getId());
			throw new EngineException(ResultStatus.BUSINESS_ERROR, "excelFileSource excelFileNameSource and excelFileNameKey can't be null. ");
		}
		...
		if(PROCESS_MODE_B.equals(processMode)) {//批量处理
			if(!StringUtils.isBlank(subBusinessFlowId)) {
		...
		// 动态属性的情况
		if (!StringUtils.isBlank(excelFileNameAttr)) {
			excelFileNameAttr = this.dynamicAttrProcess(excelFileNameAttr, businessFlowNode, context);
		}
		
		//获取excel文件名
		String excelFileName = (String)getContextObject(excelFileNameSource, excelFileNameKey, false, excelFileNameAttr, context, businessFlowNode);
		...
		if(EXCEL_FILE_SOURCE_U.equals(excelFileSource)) {
			List<MultipartFile> files = context.getMultipartFileMap().get(excelFileName);
			if(CollectionUtils.isEmpty(files)) {
				logger.error("T[{}] Excel file not found. businessflowNodeId: {}", businessFlowNode.getTenantId(), businessFlowNode.getId());
				throw new EngineException(ResultStatus.BUSINESS_ERROR, "Excel file not found. ");
			}
			
			excelFile = files.get(0);
		}else if(EXCEL_FILE_SOURCE_P.equals(excelFileSource)) {
			excelFile = (File)getContextObject(excelFileSource, excelFileName, false, null, context, businessFlowNode);
			if(excelFile == null) {
				logger.error("T[{}] Excel file not found. businessflowNodeId: {}", businessFlowNode.getTenantId(), businessFlowNode.getId());
				throw new EngineException(ResultStatus.BUSINESS_ERROR, "Excel file not found. ");
			}
		}else {
			logger.error("T[{}] invalid ExcelFileSource. businessflowNodeId: {}", businessFlowNode.getTenantId(), businessFlowNode.getId());
			throw new EngineException(ResultStatus.BUSINESS_ERROR, "invalid ExcelFileSource");
		}
		
		Object result = null;
		
		if(PROCESS_MODE_B.equals(processMode)) {//批量处理
			result = ExcelUtils.processObjectType(excelFile, sheetIndex, columnMap, startRowIndex);
		}else {//流式处理
			ExcelUtils.processObjectType(excelFile, sheetIndex, columnMap, startRowIndex, (index, rowObject)-> {
				//将循环的元素设置到过程数据中
