...
import cn.zpaas.lowcode.utils.CertCodeUtils;
...
/**
 * @author zjy
 * 身份证号码格式校验器CertCodeValidator主要用途是校验身份证号码的格式是否合法。无须增加新的配置属性。
 */
public class CertCodeValidator extends Validator {
	private static Logger logger = LoggerFactory.getLogger(CertCodeValidator.class);
	...
	protected JsonObject validateImpl(ValidateRule validateRule, Object attrValue, JsonObject srcObject, JsonObject parentObject)
		...
		//使用正则表达式进行校验
		isPass = CertCodeUtils.validateCertCode(attrValue.toString());
