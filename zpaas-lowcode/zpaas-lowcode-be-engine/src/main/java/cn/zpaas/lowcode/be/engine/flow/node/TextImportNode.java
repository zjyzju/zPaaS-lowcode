...
public class TextImportNode extends Node {
	...
	/*
	 {
		txtFileSource：txt文件的来源，包括P（过程数据：过程数据中的文件对象）；U（上传文件：上传的Multipart文件对象）
		txtFileNameSource：txt文件名的来源，包括：I（输入参数）；P（过程数据）；D（领域对象）；N（预处理产生的nodeParams）；F（固定值）
		txtFileNameKey：txt文件名的Key值，源对象类型为I（输入参数）时，表示输入参数中的Key；为D（领域对象）时，表示领域对象在context.attributedObjectMap或context.attributedObjectsMap（根据isListType的值进行区分）中的key值；为P（过程数据）时，该值表示context.values中的key值，当为N（预处理产生的nodeParams）时，Key值无效；当为F（固定值）时，为具体的值
		txtFileNameAttr：当txt文件名是指定对象的某个属性时有效，通过该字段指定对应属性的code，支持JSONPath，源对象实例属性的值只能是字符串或数字。

		processMode：处理模式，F：流式处理；B：批量处理。
		splitSymbol：分隔符，用于分隔不同的列的值
		startRowIndex：开始处理的行数，从0开始，主要用于忽略表头对应的行
		columnMappings：[//列映射，txt列号-》加载入内存后的属性名，如果未设置相应的映射，则使用列号作为属性名
			{
				columnIndex: txt列序号，
				mappingName: 映射属性名
			},
