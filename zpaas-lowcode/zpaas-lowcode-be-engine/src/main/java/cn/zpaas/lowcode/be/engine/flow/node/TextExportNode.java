...
/**
 * @author zjy 
 * TextExportNode是Text文件导出的实现类，主要用于实现Text文件的导出能力
 */
public class TextExportNode extends Node {
	private static Logger logger = LoggerFactory.getLogger(TextExportNode.class);
	...
	/*
	 {
		...
		fileName：导出文件名，为空时默认使用untitled.txt。
	...
	public void process(BusinessFlowNode businessFlowNode, BusinessFlowContext context) throws EngineException {
		...
		//导出数据到txt文件
		File result = TxtUtils.export(objectArray, columnMap, exportHeader, headerMap, splitSymbol, fileName);
