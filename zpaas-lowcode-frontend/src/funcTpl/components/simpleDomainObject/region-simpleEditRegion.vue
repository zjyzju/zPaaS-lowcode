    ...
    <el-row :gutter="4">
        ...
        <el-col :span="15" style="justify-content: right; display: inline-flex;">
            <span v-if="funcRegion.regionOperations != null" v-for="operation in funcRegion.regionOperations">
                ...
                <el-button size="small" v-if="operation.operationType == 'N' && operation.operationType != 'V'" @click="executeOperation(operation)">{{operation.name}}</el-button>
...
export default {
    ...
    watch: {        
        'regionData1': {
            handler: function(newRegionData, oldRegionData){
                ...
                interactionProcess(newRegionData, oldRegionData, this.funcDefine, this.funcRegion, null, this.regionData, this.formatRegionAttrDisplay);//联动处理
            },
            deep:true // 深度监听的参数
        }
    },
    ...
    methods: {
        ...
        formatRegionAttrDisplay() {//格式属性的显示位置以及换行等
