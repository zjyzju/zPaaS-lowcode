...
<template>
<el-dialog v-model="showFlag" title="新建指标" width="60vw">
    <template #header>
        <span class="title">新建指标</span>
    </template>
    <el-form  :model="newDataModelMetrics" label-width="120px" :rules="validateRules" ref="newDataModelMetricsForm">
        <el-row :gutter="4">
            <el-col :span="12">
                <el-form-item label="编码" required prop="code">
                    <el-input v-model="newDataModelMetrics.code"  size="small"/>
                </el-form-item>
            </el-col>
            <el-col :span="12">
                <el-form-item label="名称" required prop="name">
                    <el-input v-model="newDataModelMetrics.name"  size="small"/>
                </el-form-item>
            </el-col>
        </el-row>
        <el-row :gutter="4">
            <el-col :span="12">
                <el-form-item label="指标类型"  required prop="metricType">
                      <el-select v-model="newDataModelMetrics.metricType" class="m-2" placeholder="Select" size="small">
                           <el-option
        ...
        <el-row :gutter="4">
            <el-col :span="12">
                <el-form-item label="指标公式" required prop="metricFormula">
                    <el-input type="textarea" :rows="4" v-model="newDataModelMetrics.metricFormula"  size="small"/>
                </el-form-item>
            </el-col>
    ...
    <template #footer>
        <span class="dialog-footer">
            <el-button @click="hideDataModelMetricsCreatePage">取消</el-button>
            <el-button type="primary" @click="createDataModelMetrics()">确定</el-button>
        </span>
    </template>
</el-dialog>
</template>
...
export default {
    props: ['newDataModelMetrics','showDataModelMetricsCreate', 'dataModel'],
    
    emits: ['hideDataModelMetricsCreatePage', 'addNewDataModelMetricsToList'],
    
    setup (props, {attrs, emit, slots}) {
        const hideDataModelMetricsCreatePage = ()=> {
            emit('hideDataModelMetricsCreatePage');
        };
        
        const addNewDataModelMetricsToList = (newAttr) => {
        	emit('addNewDataModelMetricsToList', newAttr);
        };
        
        return {
        	hideDataModelMetricsCreatePage,
        	addNewDataModelMetricsToList
        };
    },
    computed: {
        showFlag: {
            get() {
                return this.showDataModelMetricsCreate;
            },
            set(value) {
    ...
    methods: {
    	createDataModelMetrics() {
            var url = "/lcdp-proxy/lowcode/platform/bi/api/dataModelMetrics/add";
            this.$refs.newDataModelMetricsForm.validate((valid, fields)=> {
                if(valid) {
                    axiosClient.post(url,this.newDataModelMetrics).then((response) => {
                    var data = response.data; 
                    if(data != null && data.status == "200" && data.data != null && data.data.id != null) {
                        ElMessage(`新增指标成功`);
                        this.addNewDataModelMetricsToList(data.data);
                    }
                });
                }else {
