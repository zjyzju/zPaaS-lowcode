...
<template>
<el-dialog v-model="showFlag" title="筛选值编辑" width="60vw">
    <template #header>
        <span class="title">筛选值编辑</span>
    </template>
    <el-form  :model="dataModelFilterValue" label-width="120px" :rules="validateRules" ref="dataModelFilterValueForm">
        <el-row :gutter="4">
            <el-col :span="12">
                <el-form-item label="标识" required prop="id">
                    <el-input v-model="dataModelFilterValue.id" readonly size="small"/>
                </el-form-item>
            </el-col>
            <el-col :span="12">
                <el-form-item label="编码" required prop="code">
                    <el-input v-model="dataModelFilterValue.code"  size="small"/>
                </el-form-item>
            </el-col>
        </el-row>
        <el-row :gutter="4">
            <el-col :span="12">
                <el-form-item label="名称" required prop="name">
                    <el-input v-model="dataModelFilterValue.name"  size="small"/>
                </el-form-item>
            </el-col>
            <el-col :span="12">
                <el-form-item label="描述"  prop="description">
                    <el-input type="textarea" :rows="4" v-model="dataModelFilterValue.description"  size="small"/>
                </el-form-item>
            </el-col>
        </el-row>   
        <el-row :gutter="4">
            <el-col :span="12">
                <el-form-item label="过滤公式" required prop="filterFormula">
                    <el-input type="textarea" :rows="4" v-model="dataModelFilterValue.filterFormula"  size="small"/>
                </el-form-item>
            </el-col>
    ...
    <template #footer>
        <span class="dialog-footer">
            <el-button @click="hideDataModelFilterValueEditPage">取消</el-button>
            <el-button type="primary" @click="saveDataModelFilterValue()">确定</el-button>
        </span>
    </template>
</el-dialog>
</template>
...
export default {
    props: ['dataModelFilterValue','showDataModelFilterValueEdit', 'dataModel'],
    
    emits: ['hideDataModelFilterValueEditPage', 'updateDataModelFilterValueToList'],
    
    setup (props, {attrs, emit, slots}) {
        const hideDataModelFilterValueEditPage = ()=> {
            emit('hideDataModelFilterValueEditPage');
        };
        
        const updateDataModelFilterValueToList = (newAttr) => {
        	emit('updateDataModelFilterValueToList', newAttr);
        };
        
        return {
        	hideDataModelFilterValueEditPage,
        	updateDataModelFilterValueToList
        };
    },
    computed: {
        showFlag: {
            get() {
                return this.showDataModelFilterValueEdit;
            },
            set(value) {
    ...
    data() {            
    	const validateRules = ref({
            ...
            "filterFormula": [
                { required: true, message: '筛选值过滤公式不能为空！', trigger: 'blur' }
            ]
        });
    ...
    methods: {
    	saveDataModelFilterValue() {
            var url = "/lcdp-proxy/lowcode/platform/bi/api/dataModelFilterValue/save";
            this.$refs.dataModelFilterValueForm.validate((valid, fields)=> {
                if(valid) {
                    axiosClient.post(url,this.dataModelFilterValue).then((response) => {
                    var data = response.data; 
                    if(data != null && data.status == "200" && data.data != null && data.data.id != null) {
                        ElMessage(`保存筛选值成功`);
                        this.updateDataModelFilterValueToList(data.data);
                    }
                });
                }else {
