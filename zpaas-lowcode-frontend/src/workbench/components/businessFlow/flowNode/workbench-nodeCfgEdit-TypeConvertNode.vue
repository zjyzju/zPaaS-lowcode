...
<template v-if="showNodeCfgEdit == 'TypeConvertNode' && nodeCfgJson != null && nodeCfgJson.isListResult != null">
	<el-row :gutter="4">
        <el-col :span="16">
    ...
    <el-row :gutter="4">
        <el-col :span="16">
            <el-form-item label="目标类型">
                <el-select v-model="nodeCfgJson.toType" class="m-2" placeholder="Select" size="small" clearable>
...
export default {
    ...
    mounted() {
        axiosClient.post("/lcdp-proxy/lowcode/platform/be/api/dict/getDictLabelValues", ['OBJECT_SOURCE_TYPE_WITH_PRE','TYPE_CONVERT_TARGET_TYPE','OBJECT_TYPE']).then((response) => {
            var data = response.data; 
            if(data != null && data.status == "200" && data.data != null) {
                this.objectSourceOptions = data.data['OBJECT_SOURCE_TYPE_WITH_PRE'];
                this.toTypeOptions = data.data['TYPE_CONVERT_TARGET_TYPE'];
