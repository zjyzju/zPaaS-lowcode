<!-- 签名/验签能力工具节点-节点配置信息-编辑页 -->

<template v-if="showNodeCfgEdit == 'SignatureUtilsNode' && nodeCfgJson != null && nodeCfgJson.isListResult != null">
	<el-row :gutter="4">
        <el-col :span="12">
        ...
        <el-col :span="12">
            <el-form-item label="算法" label-width="50px">
    ...
    <el-row :gutter="4">
        ...
        <el-col :span="12">
            <el-form-item label="密钥" label-width="50px">
    ...
    <el-row :gutter="4">
        <el-col :span="16">
            <el-form-item label="签名文本来源">
                <el-select v-model="nodeCfgJson.signTextSource" clearable class="m-2" placeholder="Select" size="small">
                    <el-option v-for="item in srcTextSourceOptions" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
        </el-col>
        <el-col :span="8">
		    <el-form-item label="" label-width="10px">
                <el-link  type="primary" @click="showAvailableDataSelectPage(srcTextSourceOptions, (contextType, contextKey)=>{this.nodeCfgJson.signTextSource=contextType;this.nodeCfgJson.signTextKey=contextKey;})" ><label>选择</label></el-link>&nbsp;
            </el-form-item>
		</el-col>
    </el-row>
    <el-row :gutter="4">
        <el-col :span="12">
            <el-form-item label="签名文本Key">
                <el-input v-model="nodeCfgJson.signTextKey" size="small" />
            </el-form-item>
        </el-col>
        <el-col :span="12">
            <el-form-item label="签名文本属性">
                <el-input v-model="nodeCfgJson.signTextAttr" size="small" />
            </el-form-item>
        </el-col>
    </el-row>
...
export default {
    ...
    mounted() {
        axiosClient.post("/lcdp-proxy/lowcode/platform/be/api/dict/getDictLabelValues", ['OBJECT_SOURCE_TYPE_WITH_PRE','SIGNATURE_OPERATION_TYPE','OBJECT_TYPE','SIGNATURE_ALGORITHM_TYPE']).then((response) => {
            var data = response.data; 
            if(data != null && data.status == "200" && data.data != null) {
                this.srcTextSourceOptions = data.data['OBJECT_SOURCE_TYPE_WITH_PRE'];
                this.operateTypeOptions = data.data['SIGNATURE_OPERATION_TYPE'];
                this.nodeResultTypeOptions = data.data['OBJECT_TYPE'];
                this.cipherAlgorithmTypeOptions = data.data['SIGNATURE_ALGORITHM_TYPE'];
            }
        }).catch(ex => {
            ElMessage.error(`加载字典数据失败!\n` + ex);
        });
        axiosClient.post("/lcdp-proxy/lowcode/platform/be/api/dict/getDictLabelValuesMore", ['SIGNATURE_OPTION_TYPE']).then((response) => {
            var data = response.data; 
            if(data != null && data.status == "200" && data.data != null) {
                this.cipherAlgorithmOptions = data.data['SIGNATURE_OPTION_TYPE'];
            }
        }).catch(ex => {
            ElMessage.error(`加载字典数据失败!\n` + ex);
        });
