<!-- 动态本地方法调用节点-节点配置信息-编辑页 -->

<template v-if="showNodeCfgEdit == 'DynamicLocalInvokeNode' && nodeCfgJson != null && nodeCfgJson.isListResult != null">
	<el-row :gutter="4">
        <el-col :span="16">
...
export default {
    ...
    data() {            
        ...
        const queryDynamicMappingCondition = ref({"mappingType":"O"});
    ...
    mounted() {
        axiosClient.post("/lcdp-proxy/lowcode/platform/be/api/dict/getDictLabelValues", ['OBJECT_SOURCE_TYPE_THREE','OBJECT_SOURCE_TYPE_FOUR','OBJECT_TYPE']).then((response) => {
            var data = response.data; 
            if(data != null && data.status == "200" && data.data != null) {
                this.paramObjectSourceOptions = data.data['OBJECT_SOURCE_TYPE_THREE'];
                this.objectSourceOptions = data.data['OBJECT_SOURCE_TYPE_FOUR'];
