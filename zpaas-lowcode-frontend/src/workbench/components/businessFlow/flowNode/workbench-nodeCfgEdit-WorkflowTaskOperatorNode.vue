<!-- 工作流任务操作节点-节点配置信息-编辑页 -->

<template v-if="showNodeCfgEdit == 'WorkflowTaskOperatorNode' && nodeCfgJson != null && nodeCfgJson.isListResult != null">
	<el-row :gutter="4">
        <el-col :span="16">
            <el-form-item label="任务来源">
                <el-select v-model="nodeCfgJson.taskIdSource" class="m-2" placeholder="Select" size="small">
                    <el-option v-for="item in taskIdSourceOptions" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
        </el-col>
        <el-col :span="8">
		    <el-form-item label="" label-width="10px">
                <el-link  type="primary" @click="showAvailableDataSelectPage(taskIdSourceOptions, (contextType, contextKey)=>{this.nodeCfgJson.taskIdSource=contextType;this.nodeCfgJson.taskIdKey=contextKey;})" ><label>选择</label></el-link>&nbsp;
            </el-form-item>
		</el-col>
    </el-row>
    <el-row :gutter="4">
        <el-col :span="12">
            <el-form-item label="任务Key">
                <el-input v-model="nodeCfgJson.taskIdKey" size="small" />
            </el-form-item>
        </el-col>
        <el-col :span="12">
            <el-form-item label="任务属性">
                <el-input v-model="nodeCfgJson.taskIdAttr" size="small" />
            </el-form-item>
        </el-col>
    </el-row>
    <el-row :gutter="4">
        <el-col :span="16">
            <el-form-item label="任务操作">
                <el-select v-model="nodeCfgJson.taskOperation" class="m-2" placeholder="Select" size="small">
                    <el-option v-for="item in taskOperationOptions" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
        </el-col>
...
export default {
    ...
    data() {            
    	const taskIdSourceOptions = ref(null);
        
        const taskOperationOptions = ref(null);
    ...
    mounted() {
        axiosClient.post("/lcdp-proxy/lowcode/platform/be/api/dict/getDictLabelValues", ['OBJECT_SOURCE_TYPE_ALL','OBJECT_SOURCE_TYPE_THREE','OBJECT_TYPE','PROCESS_TASK_OPERATION_TYPE']).then((response) => {
            ...
            if(data != null && data.status == "200" && data.data != null) {
                this.taskIdSourceOptions = data.data['OBJECT_SOURCE_TYPE_ALL'];
                this.processVariablesSourceOptions = data.data['OBJECT_SOURCE_TYPE_THREE'];
                this.nodeResultTypeOptions = data.data['OBJECT_TYPE'];
                this.taskOperationOptions = data.data['PROCESS_TASK_OPERATION_TYPE'];
            }
        }).catch(ex => {
