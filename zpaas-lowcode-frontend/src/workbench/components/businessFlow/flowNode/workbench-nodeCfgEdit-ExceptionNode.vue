<!-- ExceptionNode-节点配置信息-编辑页 -->

<template v-if="showNodeCfgEdit == 'ExceptionNode' && nodeCfgJson != null && nodeCfgJson.isListResult != null">
    <el-row :gutter="4">
        <el-col :span="16">
            <el-form-item label="返回状态">
                <el-select v-model="nodeCfgJson.resultStatus" class="m-2" placeholder="Select" size="small">
	...
	<el-row :gutter="4">
        <el-col :span="24">
            <el-form-item label="详细信息">
                <el-input v-model="nodeCfgJson.message" size="small" />
            </el-form-item>
        </el-col>
    </el-row>
...
export default {
    ...
    data() {            
    	...
    	const resultStatusOptions = ref(null);
    ...
    mounted() {
        axiosClient.post("/lcdp-proxy/lowcode/platform/be/api/dict/getDictLabelValues", ['RESULT_EXCEPTION_TYPE','OBJECT_TYPE']).then((response) => {
            var data = response.data; 
            if(data != null && data.status == "200" && data.data != null) {
                this.resultStatusOptions = data.data['RESULT_EXCEPTION_TYPE'];
